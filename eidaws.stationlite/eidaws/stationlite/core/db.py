...
class StationLiteDBEngineError(Error):
    """General purpose eidaws-stationlite DB engine error ({})."""
...
class ScopedSession:
    ...
    The user finally obtains a scoped session as described under
...
def clean(session, timestamp):
    ...
    for vnet_not_active in vnets_all - vnets_active:
        logger.debug(f"Deleting VNET {vnet_not_active!r}")
