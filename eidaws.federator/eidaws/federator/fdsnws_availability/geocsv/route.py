...
from eidaws.federator.fdsnws_availability.geocsv.view import (
    AvailabilityQueryView,
    AvailabilityExtentView,
)
from eidaws.federator.settings import FED_STATIC, FED_AVAILABILITY_PATH_GEOCSV
...
FED_AVAILABILITY_GEOCSV_PATH_QUERY = (
    FED_AVAILABILITY_PATH_GEOCSV + "/" + FDSNWS_QUERY_METHOD_TOKEN
)
FED_AVAILABILITY_GEOCSV_PATH_EXTENT = (
    FED_AVAILABILITY_PATH_GEOCSV + "/" + FDSNWS_EXTENT_METHOD_TOKEN
)
...
def setup_routes(app, static=False):
    ...
    routes = [
        web.view(FED_AVAILABILITY_GEOCSV_PATH_QUERY, AvailabilityQueryView),
