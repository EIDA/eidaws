...
class FDSNHTTPError(HTTPException):
    ...
    def __init__(self, request, request_submitted=None, documentation_uri=None,
                 service_version=None, error_desc_long=None):
        ...
        text = self.make_error_message(
            self.status_code, self.error_desc_short, error_desc_long,
            documentation_uri, request.url, request_submitted.isoformat(),
            service_version)

        super().__init__(text=text, content_type=self.CONTENT_TYPE)

    @staticmethod
    def make_error_message(status_code, description_short, description_long,
        ...
        return ERROR_MESSAGE_TEMPLATE % (
            status_code, description_short, description_long,
            documentation_uri, request_url, request_time, service_version)
